...
open class Strava {
    ...
    open static func request(_ method: HTTPMethod, authenticated: Bool, path: String, params: [String: Any]?, completionHandler: ((_ response: Any?, _ error: NSError?) -> ())?) -> URLSessionTask? {
        ...
        if sharedInstance.alternateRequestor == nil {
            sharedInstance.alternateRequestor = DefaultRequestor()
        }
    ...
    @discardableResult
    open static func request(_ method: HTTPMethod, fromFile: URL, path: String, params: [String: Any]?, completionHandler: ((_ response: Any?, _ error: NSError?) -> ())?) -> URLSessionTask? {
        if isDebugging {
            debugPrint("Method: \(method.rawValue), Path: \(path), Authenticated: true")
            if let params = params {
                debugPrint("Params: \(params)")
            }
        }
        ...
        if let alternateRequestor = sharedInstance.alternateRequestor {
            alternateRequestor.request(method, fromFile: fromFile, path: path, params: params, completionHandler: completionHandler)
        }
        
        return sharedInstance.defaultRequestor.request(method, fromFile: fromFile, path: path, params: params, completionHandler: completionHandler)
    }
    
    
    open static func request_upload(_ url: String, params: [String : String]?, file: URL, completionHandler: ((_ response: Any?, _ error: NSError?) -> ())?) {
        if sharedInstance.alternateRequestor == nil {
            sharedInstance.alternateRequestor = DefaultRequestor()
        }
        if let alternateRequestor = Strava.sharedInstance.alternateRequestor {
            alternateRequestor.uploadFitFile(url: url, params: params, file: file, completionHandler: completionHandler)
        }
    }
